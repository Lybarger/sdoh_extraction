...
# from config.constants import NO_CONCAT, CONCAT_LOGITS, CONCAT_PROBS, LABEL_BIAS
...
def cfg():
    ...
    # source: str defining source corpus name
    source = "sdoh_challenge"

    # source_file: str defining path to source corpus
    ...
    subtype_default = C.SUBTYPE_DEFAULT
    ...
    spert_types_config["entities"] = entity_types
    ...
    args_by_event_type = C.ARGUMENTS_BY_EVENT_TYPE
    ...
    #   ex. ["Assertion", "Indication_Type", "Size"]
    labeled_args = [C.STATUS_TIME, C.STATUS_EMPLOY, C.TYPE_LIVING]
...
def main(source_file, destination, config_path, model_config, spert_path, \
        ...
        types_path, entity_types,
        # argument_source, argument_target, default_subtype_value,
        attr_type_map, arg_role_map, skip_dup_trig,
        labeled_args, scoring, argument_types, subtype_layers, swapped_spans,
        save_brat, event_types, subtype_default, args_by_event_type):
    ...
    # for arg in swapped_spans:
    #     corpus.swap_spans(arg, C.TRIGGER)
    #
    ...
    # predict_corpus.prune_invalid_connections(args_by_event_type, path=destination)
